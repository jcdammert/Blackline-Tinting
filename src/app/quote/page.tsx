...
export default function Quote() {
  ...
  function validate(s: number): string[] {
    ...
    if (s === 4) {
      if (!timeline) errs.push('Please select a timeline')
    }
    return errs
  }
  ...
  async function handleSubmit() {
    ...
    try {
      await fetch(WEBHOOK, {
        ...
        body: JSON.stringify({
          firstName, lastName, phone, email, vehicle, service,
          tintWindows: tintWindows.join(', '),
          hasExistingTint,
          existingTintWindows: existingTintWindows.join(', '),
          ppfAreas: ppfAreas.join(', '),
          ceramicAreas: ceramicAreas.join(', '),
          timeline, appointmentTime, message,
        }),
      })
      setSubmitted(true)
    } catch {
            ...
            {submitted ? (
              ...
              <div>
                ...
                {step === 2 && (
                  <div className="form-grid">
                    <div className="form-group full">
                      <input className="form-input" placeholder="Vehicle Make / Model / Year" value={vehicle} onChange={e => setVehicle(e.target.value)} />
                    </div>
                    <div className="form-group full">
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px' }}>
                        {(['tinting', 'ppf', 'ceramic'] as Service[]).map(s => (
                          <div key={s} onClick={() => setService(s)}
                            style={{ border: service === s ? '2px solid var(--blue)' : '1px solid var(--border)', borderRadius: '8px', padding: '16px 8px', cursor: 'pointer', textAlign: 'center', background: service === s ? 'rgba(55,138,221,0.08)' : 'transparent' }}>
                    ...
                    <div className="form-group full" style={{ display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    {service === 'tinting' && (
                      <>
                        <div className="form-group full">
                          ...
                          {['Full vehicle (all windows)', 'Front two windows only', 'All side windows and rear windshield', 'Windshield strip (sun strip)', 'Full windshield', 'Not sure yet, I\'d like a recommendation'].map(opt => (
                            <label key={opt} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px', fontSize: '14px', cursor: 'pointer' }}>
                              <input type="checkbox" checked={tintWindows.includes(opt)} onChange={() => toggleCheck(tintWindows, setTintWindows, opt)} />
                              {opt}
                            </label>
                          ))}
                        </div>
                        <div className="form-group full" style={{ borderTop: '1px solid var(--border)', paddingTop: '16px' }}>
                          ...
                          <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
                            {[true, false].map(val => (
                              <button key={String(val)} onClick={() => setHasExistingTint(val)}
                                style={{ padding: '6px 20px', borderRadius: '6px', border: hasExistingTint === val ? '2px solid var(--blue)' : '1px solid var(--border)', background: hasExistingTint === val ? 'rgba(55,138,221,0.08)' : 'transparent', cursor: 'pointer', fontSize: '13px' }}>
                                {val ? 'Yes' : 'No'}
                              </button>
                            ))}
                          </div>
                          {hasExistingTint && (
                            <div style={{ background: 'var(--bg-secondary)', borderRadius: '8px', padding: '12px' }}>
                              ...
                              {['All windows', 'Front two windows', 'Rear windshield', 'Side windows only', 'Not sure'].map(opt => (
                                <label key={opt} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px', fontSize: '14px', cursor: 'pointer' }}>
                                  <input type="checkbox" checked={existingTintWindows.includes(opt)} onChange={() => toggleCheck(existingTintWindows, setExistingTintWindows, opt)} />
                                  {opt}
                                </label>
                              ))}
                            </div>
                    ...
                    {service === 'ppf' && (
                      <div className="form-group full">
                        ...
                        {['Full front (hood, fenders, bumper, mirrors)', 'Hood only', 'Full vehicle', 'Rocker panels / door edges', 'Not sure, I\'d like a recommendation'].map(opt => (
                          <label key={opt} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px', fontSize: '14px', cursor: 'pointer' }}>
                            <input type="checkbox" checked={ppfAreas.includes(opt)} onChange={() => toggleCheck(ppfAreas, setPpfAreas, opt)} />
                            {opt}
                          </label>
                        ))}
                      </div>
                    ...
                    {service === 'ceramic' && (
                      <div className="form-group full">
                        ...
                        {['Full exterior paint', 'Wheels', 'Windows', 'Full vehicle package', 'Not sure, I\'d like a recommendation'].map(opt => (
                          <label key={opt} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px', fontSize: '14px', cursor: 'pointer' }}>
                            <input type="checkbox" checked={ceramicAreas.includes(opt)} onChange={() => toggleCheck(ceramicAreas, setCeramicAreas, opt)} />
                            {opt}
                          </label>
                        ))}
                      </div>
                  ...
                  <div className="form-grid">
                    <div className="form-group full">
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                        {[
                          { label: 'ASAP', sub: 'Within a few days' },
                          { label: 'This week', sub: 'Before the weekend' },
                          { label: '1–2 weeks', sub: 'No big rush' },
                          { label: 'Just researching', sub: 'Checking prices' },
                        ].map(({ label, sub }) => (
                          <div key={label} onClick={() => setTimeline(label)}
                            style={{ border: timeline === label ? '2px solid var(--blue)' : '1px solid var(--border)', borderRadius: '8px', padding: '10px 12px', cursor: 'pointer', background: timeline === label ? 'rgba(55,138,221,0.08)' : 'transparent' }}>
                    ...
                    <div className="form-group full">
                      <input className="form-input" placeholder="Preferred appointment time (optional)" value={appointmentTime} onChange={e => setAppointmentTime(e.target.value)} />
                    </div>
                    <div className="form-group full">
                      <textarea className="form-textarea" placeholder="Additional details (optional)" value={message} onChange={e => setMessage(e.target.value)} />
                    </div>
                    <div className="form-group full" style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <button className="btn btn-secondary" onClick={back}>← Back</button>
